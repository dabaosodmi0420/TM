...
@interface YT_PageControl ()
@end

@implementation YT_PageControl
{
@private
...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect
{
    ...
    UIImage *image = nil;
    CGFloat indicatorWidth = _measuredIndicatorWidth;
    
    for (NSUInteger i = 0; i < _numberOfPages; i++) {
        indicatorWidth = _measuredIndicatorWidth;
        if (i == _displayedPage) {
        ...
        if (image) {
            ...
            if (i == _displayedPage && _indicatorType == SMPageControlCurrentIndicatorTypeGreater) {
               
                indicatorWidth = _measuredIndicatorWidth * 3;
                if (self.indicatorW) {
                    indicatorWidth =self.indicatorW;
                }
                [self drawRoundRect:context rect:CGRectMake(xOffset, yOffset, indicatorWidth, _measuredIndicatorHeight) fillColor:fillColor];
            }else {
                CGContextFillEllipseInRect(context, CGRectMake(xOffset, yOffset, indicatorWidth, _measuredIndicatorHeight));
            }
        }
        
        xOffset += indicatorWidth + _indicatorMargin;
    }
...
- (void)drawRoundRect:(CGContextRef)context rect:(CGRect)rect fillColor:(UIColor *)fillColor {
    CGContextSetFillColorWithColor(context, fillColor.CGColor);
    CGFloat leftX = rect.origin.x;
    CGFloat rightX = leftX + rect.size.width;
    CGFloat topY = rect.origin.y;
    CGFloat bottomY = topY + rect.size.height;
    CGFloat radius = rect.size.height / 2.0;

    CGContextMoveToPoint(context, leftX + radius, topY);

    CGContextAddLineToPoint(context, rightX - radius, topY);
    CGContextAddArc(context, rightX - radius, topY + radius, radius, -0.5 * M_PI, 0.0, 0);

    CGContextAddLineToPoint(context, rightX, bottomY - radius);
    CGContextAddArc(context, rightX - radius, bottomY - radius, radius, 0.0, 0.5 * M_PI, 0);

    CGContextAddLineToPoint(context, leftX + radius, bottomY);
    CGContextAddArc(context, leftX + radius, bottomY - radius, radius, 0.5 * M_PI, M_PI, 0);

    CGContextAddLineToPoint(context, leftX, bottomY);
    CGContextAddArc(context, leftX + radius, bottomY - radius, radius, M_PI, 1.5 * M_PI, 0);
...
- (CGSize)sizeForNumberOfPages:(NSInteger)pageCount
{
    ...
    if (_indicatorType == SMPageControlCurrentIndicatorTypeGreater) {
        indicatorSpace = (pageCount + 2) * _measuredIndicatorWidth;
    }
